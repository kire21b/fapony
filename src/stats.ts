...
export function cmdStats(_args: string[]): void {
  ...
  console.log(
    `runs: ${runs.length}  (${Object.entries(byStatus)
      .map(([k, v]) => `${k}=${v}`)
      .join(", ")})`,
  );
  console.log(
    `pass rate: ${(passRate * 100).toFixed(0)}%  stall rate: ${(stallRate * 100).toFixed(0)}%`,
  );
  console.log(
    `avg rounds to pass: ${avgRounds.toFixed(1)}  avg time to pass: ${avgMinutes.toFixed(0)}m`,
  );
  ...
  if (total.spawns > 0) {
    const usd =
      total.usd_estimate !== null
        ? ` (~$${total.usd_estimate.toFixed(4)} est.)`
        : "";
    console.log(
  ...
  for (const [wt, rs] of Object.entries(byWorktree)) {
    ...
    console.log(
      `  ${wt.padEnd(8)} | ${String(rs.length).padStart(4)} | ${String(p).padStart(6)} | ${String(s).padStart(7)}`,
    );
  }
}
