...
> **Status:** 🚧 in-progress · **Owner:** delamind · **Created:** 2026-09-09 · **Amended:** 2026-09-09
...
- dogfood [PLAN-verification-report.md](PLAN-verification-report.md) step 6 บน standalone repo
  ที่ไม่ใช้ fapony loop — วัด adoption friction จริง (agent จบ flow ได้โดยไม่ถามผู้สร้าง)
- ปิด PLAN-readme-launch เป็น superseded

**ไม่ทำ:**
- ไม่ลบ/ทุบ loop code — loop ยัง build ผ่านและใช้ dogfood ต่อ (แค่เลิกเป็น headline)
...
- dogfood standalone repo มีบันทึก: จำนวนขั้นตอนจนได้ report, จุดที่ agent ติด, gap list
...
| ผู้ใช้ loop เดิมหาวิธีใช้ไม่เจอ | ต่ำ | ความงง | เก็บ loop ครบใน section "Optional loop" — เปลี่ยนแค่ลำดับ |

## 6. ขั้นตอน (เรียงลำดับ แต่ละขั้น verify ได้)

1. **scaffold evidence.json ใน [src/init.ts](../src/init.ts) + test** → verify: init ใน temp dir
   ได้ `.fapony/evidence.json` ตาม shape ใน [SPEC-verification-report.md](../spec/SPEC-verification-report.md)
   + init ซ้ำไม่ทับ + `bun test` เขียว
2. **README refactor MCP-first** → verify: คำสั่งตรง fapony.ts, MCP table ครบ 6 tools,
   ทุกลิงก์เปิดได้
3. **dogfood step 6 บน standalone repo** → verify: มีบันทึก step count + จุดที่ agent ติด +
   gap list (ไม่มีบันทึก = ไม่ได้ลองจริง)
...
- [PLAN-verification-report.md](PLAN-verification-report.md) — step 6 (dogfood standalone) ย้ายมาอยู่ใน scope นี้
...
- [ROADMAP.md](../ROADMAP.md) §เป้าหมาย — north star ที่ pitch ใหม่ align ด้วย
